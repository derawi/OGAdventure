...
export const InventoryContextApp = ({ children }) => {
  ...
  const assignSharedInventory = React.useCallback(async () => {
    ...
    // Todo: Enable here again, however this will thorw an error
    return;
  ...
  function prepareInventory(tokenID) {
    ...
    const rarityBasicSet = new Contract(
  ...
  const updateInventories = React.useCallback(
    async (adventurers) => {
      ...
      for (let index = 0; index < adventurers.length; index++) {
        ...
        if (chunckLen === 0) chunckLen = calls.length;
  ...
  React.useEffect(() => {
    if (!isLoaded || initialFetchSet) return;
