...
export const RarityContextApp = ({ children }) => {
  ...
  https://explorer.evm.testnet.shimmer.network/api
		?module=account
		&action=tokentx
  ...
  async function setRarity(tokenID, multicallResult) {
    ...
    const _adventurer = {
      ...
      nextAdventure:
        Number(adventurer["_log"]) === 0
          ? "Now"
          : dayjs(new Date(Number(adventurer["_log"]) * 1000)).from(dayjs(new Date(chainTime * 1000))),
  ...
  async function fetchRarity() {
    const { result } = await fetcher(`https://explorer.evm.testnet.shimmer.network/api
			?module=account
			&action=tokentx
			&contractaddress=${process.env.RARITY_ADDR}
			&address=${address}`);
  ...
  useEffect(() => {
    if (loaded === false) setTimeout(() => (!active ? set_loaded(true) : null), 5000); //5s before unlock
  }, [active, loaded]);
  ...
  return (
    <RarityContext.Provider
      ...
      <ModalSelectAdventurer
        ...
        onSelect={(adventurer) => set_currentAdventurer(adventurer)}></ModalSelectAdventurer>
    </RarityContext.Provider>
  );
};
