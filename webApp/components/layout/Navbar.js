		...
		if (!active) {
			return (
				<button
					...
					className={
						'inline-flex items-center py-2 px-3 text-xs font-semibold leading-4 whitespace-nowrap cursor-pointer'
					}>
	...
	return (
		<>
			<div className={'block md:hidden'}>
				<div>
					<div className={'flex justify-between items-start pt-2 pb-4'}>
						<h1 className={'text-lg font-medium text-plain'}>{'Rarity Extended'}</h1>
							...
							{active ? (
								<button
									...
									<svg
										...
										className={'mt-1 w-4 h-4 text-plain'}
										role={'img'}
			...
			<div className={'hidden md:block'}>
				<nav
					className={
						'flex relative flex-col justify-between pb-0 mb-4 w-full border-b-4 border-black dark:border-dark-100 md:flex-row md:pb-4 md:mb-4 md:h-20'
					}>
					<div
						className={
							'flex flex-row justify-start items-center w-full text-lg whitespace-normal md:w-3/12 md:whitespace-nowrap'
						}>
						<div className={'w-full'}>
							...
							<Link href={'/'}>
								<div>
									...
									<p className={'text-sm transition-opacity text-plain'}>{'By Rarity Extended'}</p>
								</div>
							</Link>
						</div>
						<div className={'flex flex-row justify-end items-center w-full md:hidden'}>{renderWalletButton()}</div>
					</div>

					<div className={'flex flex-row justify-start items-center mt-3 w-3/12 md:justify-end md:mt-0'}>
						<div className={'hidden flex-row justify-end items-center pl-6 ml-6 md:flex'}>{renderWalletButton()}</div>
					</div>
					<ModalLogin open={modalLoginOpen} set_open={set_modalLoginOpen} />
				</nav>
			</div>
