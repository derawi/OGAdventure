...
function AttributeSetter({adventurer, value, attributesAreInit, updateAttribute, set_updateAttribute, name}) {
	...
	function increaseStrength() {
		attributes.increaseStrength({provider, adventurer: adventurer.tokenID}, ({error}) => {
			if (error) return console.error(error);
			updateRarity(adventurer.tokenID);
		});
	}
	function increaseDexterity() {
		attributes.increaseDexterity({provider, adventurer: adventurer.tokenID}, ({error}) => {
			if (error) return console.error(error);
			updateRarity(adventurer.tokenID);
		});
	}
	function increaseConstitution() {
		attributes.increaseConstitution({provider, adventurer: adventurer.tokenID}, ({error}) => {
			if (error) return console.error(error);
			updateRarity(adventurer.tokenID);
		});
	}
	function increaseIntelligence() {
		attributes.increaseIntelligence({provider, adventurer: adventurer.tokenID}, ({error}) => {
			if (error) return console.error(error);
			updateRarity(adventurer.tokenID);
		});
	}
	function increaseWisdom() {
		attributes.increaseWisdom({provider, adventurer: adventurer.tokenID}, ({error}) => {
			if (error) return console.error(error);
			updateRarity(adventurer.tokenID);
		});
	}
	function increaseCharisma() {
		attributes.increaseCharisma({provider, adventurer: adventurer.tokenID}, ({error}) => {
			if (error) return console.error(error);
			updateRarity(adventurer.tokenID);
		});
	}
	...
	if (!attributesAreInit) {
		return (
			<div className={'flex flex-row items-center'}>
				<div
					onClick={() => {
						if (updateAttribute[name] === value || !(updateAttribute.remainingPoints >= 0)) return;
					...
					className={`text-xl -m-2 p-2 text-center ${
						updateAttribute[name] === value || !(updateAttribute.remainingPoints >= 0)
							? 'opacity-0'
							: 'cursor-pointer opacity-20 hover:opacity-100 transition-opacity select-none'
					}`}>
					{'-'}
				</div>
				...
				<div
					onClick={() => {
						if (!(updateAttribute.remainingPoints >= 0)) return;
					...
					className={`text-xl -m-2 p-2 text-center ${
						getNextRemainingPoints() < 0 || !(updateAttribute.remainingPoints >= 0)
							? 'opacity-0'
							: 'cursor-pointer opacity-20 hover:opacity-100 transition-opacity select-none'
					}`}>
					{'+'}
	...
	return (
		<div className={'flex flex-row items-center'}>
			<p className={'px-3 text-xl'}>{value}</p>
			<div className={'p-2 -m-2 text-xl text-center opacity-0'}>{'+'}</div>
		</div>
	);
}
	...
	function renderSaveButtons() {
		if (!attributesAreInit) {
			return (
				<div
					...
					className={`w-full text-sm font-bold text-highlight ${
						updateAttribute.remainingPoints === 0 ? 'cursor-pointer animate-pulse' : ''
					}`}>
		...
		if (updateAttribute.extraPoints > updateAttribute.extraPointsSpents) {
			...
			return (
				<div
					className={`w-full text-sm font-bold text-highlight ${
						extraRemainingPoints === 0 ? 'cursor-pointer animate-pulse' : ''
					}`}>
		...
		return <div className={'w-full text-sm font-bold opacity-0 text-highlight'}>{'Save attributes'}</div>;
	}
		...
		<div className={'flex relative flex-col-reverse items-center p-4 mt-auto w-full md:static md:flex-col md:p-0'}>
			<div className={'grid grid-cols-1 gap-x-16 gap-y-2 mt-4 w-full md:grid-cols-2 md:mt-0'}>
				...
				<div className={'flex flex-row justify-between items-center'}>
					...
					<FakeAttributeSetter
						value={attack_bonus(adventurer.class, adventurer.attributes.strength, adventurer.level)}
					/>
				</div>
				<div className={'flex flex-row justify-between items-center'}>
					<div className={'flex flex-row items-center opacity-60'}>
			...
			<div className={'absolute -top-4 right-4 md:top-0 md:right-0'}>{renderSaveButtons()}</div>
