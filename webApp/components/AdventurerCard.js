...
function WrapperMinimal({
	adventurer,
	set_currentAdventurer,
	favoritesAdventurers,
	set_favoritesAdventurers,
	raritySkin,
}) {
				...
				<div className={'flex relative flex-col justify-between items-center p-4 w-full box'}>
					...
					<div
						onClick={(e) => {
							...
							e.stopPropagation();
							set_favoritesAdventurers(
								favoritesAdventurers.includes(adventurer.tokenID)
									? favoritesAdventurers.filter((id) => id !== adventurer.tokenID)
									: [...favoritesAdventurers, adventurer.tokenID]
							);
						}}
						className={`absolute transition-colors left-4 top-4 cursor-pointer ${
							favoritesAdventurers.includes(adventurer.tokenID)
								? 'text-highlight'
								: 'text-gray-200 hover:text-light-primary dark:text-dark-600 dark:hover:text-dark-primary'
						}`}>
						<svg
							width={20}
							height={20}
					...
					<div className={'px-4'}>
						<button onClick={() => set_currentAdventurer(adventurer)} className={'mt-4 button-highlight-outline'}>
				...
				<div className={'flex relative flex-row justify-between items-center p-2 w-full box'}>
					...
					<div>
						...
						<div
							...
							className={`absolute transition-colors left-2 top-2 cursor-pointer ${
								favoritesAdventurers.includes(adventurer.tokenID)
									? 'text-highlight'
									: 'text-gray-200 hover:text-light-primary dark:text-dark-600 dark:hover:text-dark-primary'
							}`}>
						...
						<div className={'px-4'}>
							<button
								...
								className={
									'flex py-2 px-4 mt-4 w-full text-center opacity-60 cursor-pointer bg-600 hover-bg-900 flex-center text-plain'
								}>
