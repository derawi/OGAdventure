	...
	return (
		<div className={'flex relative flex-col justify-between items-center p-4 w-full box'}>
			<div className={'text-center'}>
				<p className={'text-xl font-bold text-center text-plain dark:text-opacity-70'}>{'Recruit'}</p>
				<p className={'mb-4 text-sm text-black dark:text-dark-100'}>{'Unknown'}</p>
			</div>
				...
				<div className={'col-span-12 mt-4 md:mt-8'}>
					<div className={'grid grid-cols-1 gap-2 md:grid-cols-4 md:gap-4'}>
						{([...Object.values(rarities || {})] || [])
							.sort((a, b) => {
								if (favoritesAdventurers.includes(a.tokenID)) return -1;
								if (favoritesAdventurers.includes(b.tokenID)) return 1;
								return 0;
							})
							.map((adventurer, i) => {
								return (
