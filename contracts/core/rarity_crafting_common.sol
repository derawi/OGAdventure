...
interface rarity_skills {
    function get_skills(
        uint _summoner
    ) external view returns (uint8[36] memory);
}
...
interface codex_items_armor {
    function get_proficiency_by_id(
        uint _id
    ) external pure returns (string memory description);
...
interface codex_items_weapons {
    ...
    function get_encumbrance_by_id(
        uint _id
    ) external pure returns (string memory description);

    function get_damage_type_by_id(
        uint _id
    ) external pure returns (string memory description);
...
contract rarity_crafting is ERC721Enumerable {
    ...
    constructor(
        ...
        rarity_crafting_materials_i __craft_i,
        rarity_gold __gold,
        rarity_skills __skills,
        codex_base_random __random,
        codex_items_goods __goods,
        codex_items_armor __armor,
        codex_items_weapons __weapons
    ) {
        _rm = __rm;
        _attr = __attr;
        _craft_i = __craft_i;
        ...
        SUMMMONER_ID = __rm.next_summoner();
        __rm.summon(11);
    }
    ...
    function modifier_for_attribute(
        uint _attribute
    ) public pure returns (int _modifier) {
        if (_attribute == 9) {
            return -1;
        }
    ...
    function craft_skillcheck(
        uint _summoner,
        uint _dc
    ) public view returns (bool crafted, int check) {
    ...
    ) external view returns (bool crafted, int check, uint cost, uint dc) {
